...
export class AdminPostService {
  ...
  getPost(slug,draft=false) {
    if (draft) return this.db.list('/draft-posts/').snapshotChanges()
    .map(itemChanges => 
      itemChanges.map(itemChange => 
        ({ $key: itemChange.payload.key, ...itemChange.payload.val() })
      )
    ).map(posts => posts[0])
    return this.db.list('/posts/',ref => ref.orderByChild('postOption/slug').equalTo(slug)).snapshotChanges()
    .map(itemChanges => 
      itemChanges.map(itemChange => 
        ({ $key: itemChange.payload.key, ...itemChange.payload.val() })
      )
  ...
  deletePost(post) {
    ...
    .then(
      ...
      (e) => this.snackBarService.openSnackBar("Lỗi: "+e.message,"Đóng",7000)
      )
    else {
    return this.db.object('/').update(updates)
    .then(
  ...
  pinPost(post,value) {
    ...
    .then(
      ...
      (e) => this.snackBarService.openSnackBar("Lỗi: "+e.message,"Đóng",3000)
    )
  }
  ...
  getCustomPosts(orderByChild?,equalTo?,limitToFirst?) {
    ...
    return this.db.list('/posts', ref => ref.orderByChild(orderByChild || null).equalTo(equalTo || null).limitToFirst(limitToFirst || 1000)).snapshotChanges()
    .map(itemChanges => 
      itemChanges.map(itemChange => 
        ({ $key: itemChange.payload.key, ...itemChange.payload.val() })
      )
  ...
  getDraftPosts() {
    this.eventService.addEvent('listLoading',true);
    return this.db.list('/draft-posts').snapshotChanges()
    .map(itemChanges => 
      itemChanges.map(itemChange => 
        ({ $key: itemChange.payload.key, ...itemChange.payload.val() })
      )
