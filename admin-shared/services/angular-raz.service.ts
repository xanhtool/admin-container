...
@Injectable()
export class AngularRazService {

  constructor() { }

  resize(fileTarget, maxWidth, maxHeight) {
    ...
    imageTarget.onload = (image) => {
        ...
        if(newHeight===currentHeight && newWidth === currentWidth){
           compressedFileSubject.next(<File>fileTarget)
        }
        else{
            // To canvas
            ...
            .then((blob:Blob) => {
                ...
                compressedFileSubject.next(<File>file)
            })
            .catch(error => {
                compressedFileSubject.error({ compressedFile: fileTarget, error });
            })
        }
    }
  ...
  timeAtStart:any;
  initialFile:any;
  compressImage(file: File, maxSizeInMB: number, ignoreAlpha: boolean = false, logExecutionTime: boolean = false): Observable<any> {
      ...
      imageTarget.onload = () => {
              ...
              cvs.width = imageTarget.naturalWidth  || imageTarget.width;
              cvs.height = imageTarget.naturalHeight || imageTarget.height; 
              ...
              console.log('start getCompressedFile',ctx)
              this.getCompressedFile(cvs, 50, maxSizeInMB, 1).then((compressedFile) => {
  ...
  getCompressedFile(cvs: HTMLCanvasElement, quality: number, maxSizeInMB: number, currentStep: number): Promise<File> {
  ...
  getCalculatedQuality(blob: Blob, quality: number, maxSizeInMB: number, currentStep: number): number {
  ...
  checkCompressionStatus(cvs: HTMLCanvasElement, blob: Blob, quality: number, maxSizeInMB: number, currentStep: number, newQuality: number): Promise<any> {
  ...
  isImgUsingAlpha(imageData): boolean {
  ...
  logExecutionTime(logExecutionTime: boolean): void {
      if (logExecutionTime) {
